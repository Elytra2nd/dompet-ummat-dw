...
// ── Wilayah ────────────────────────────────────────────────────────────────
function DemografiWilayah({ appliedFilter, programFilter = PROGRAM_FILTER_DEFAULT }: DemografiProps) {
  const [data, setData] = useState<WilayahItem[]>([])
  const [loading, setLoading] = useState(true)
  const [history, setHistory] = useState<{ kab: string | null; kec: string | null }>({ kab: null, kec: null })
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        // Gabung filter waktu + program + wilayah drill-down
    ...
    fetchData()
  }, [appliedFilter, programFilter?.program, history])
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-8 h-full border border-slate-100 transition-all">
      <div className="flex justify-between items-start mb-6">
        ...
        {history.kab && (
          <button onClick={goBack} className="bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold px-4 py-2 rounded-xl transition-all active:scale-95">
            ← Kembali
          </button>
        )}
      </div>
        ...
        <div className="flex flex-col xl:flex-row xl:items-center gap-6">
          <div className="relative w-full xl:w-[260px] h-[240px] xl:flex-shrink-0 flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={data} dataKey="total" nameKey="label"
                  cx="50%" cy="50%" innerRadius={70} outerRadius={110}
                  paddingAngle={4} minAngle={6} stroke="none"
                  onClick={onPieClick}
                >
                  {data.map((_, index) => (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              ...
              <p className="text-3xl font-black text-slate-900">{total.toLocaleString('id-ID')}</p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-2">
            {legendData.map((entry, index) => {
              ...
              const color = entry.label === 'Lainnya' ? '#94a3b8' : COLORS[index % COLORS.length]
              return (
                <div
                  key={entry.label}
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="text-lg font-extrabold text-slate-900">{entry.total.toLocaleString('id-ID')}</p>
...
// ── Gender ─────────────────────────────────────────────────────────────────
function DemografiGender({ appliedFilter, programFilter = PROGRAM_FILTER_DEFAULT }: DemografiProps) {
  ...
  useEffect(() => {
    const fetchGenderStats = async () => {
      ...
      try {
        const params = appendProgramParams(buildQueryParams(appliedFilter), programFilter)
        const res = await fetch(`/api/mustahik/gender?${params.toString()}`, { cache: 'no-store' })
        ...
        console.error('[DemografiGender] Error:', err)
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 space-y-6">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center gap-6">
        <div className="w-full xl:w-[260px] h-[240px] xl:flex-shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={data} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={70} outerRadius={110} strokeWidth={3} stroke="#ffffff">
        ...
        <div className="flex-1 space-y-3">
          {data.map((item) => {
            ...
            return (
              <div key={item.raw} className="flex items-center gap-3 p-3 rounded-2xl border border-slate-100 transition-all hover:translate-x-1" style={{ background: bg }}>
                <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: color }} />
                <div className="flex-1 min-w-0">
                ...
                <div className="text-right">
                  <p className="text-lg font-extrabold text-slate-900">{item.value.toLocaleString('id-ID')}</p>
                  <p className="text-xs text-slate-400 font-medium">{pct}%</p>
                </div>
              </div>
            )
...
// ── Export utama ───────────────────────────────────────────────────────────
export default function DemografiGabungan({ appliedFilter, programFilter }: DemografiProps) {
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      <DemografiGender appliedFilter={appliedFilter} programFilter={programFilter} />
      <DemografiWilayah appliedFilter={appliedFilter} programFilter={programFilter} />
    </div>
  )
}