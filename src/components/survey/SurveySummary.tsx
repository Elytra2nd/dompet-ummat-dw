...
export default function SurveySummary({ score, status }: SurveySummaryProps) {
  ...
  return (
    <Card className={`overflow-hidden border-2 transition-all duration-500 ${config.borderColor} ${config.bgColor}`}>
      <CardContent className="p-6">
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div className="relative flex items-center justify-center">
            <div className={`text-5xl font-black tracking-tighter ${config.color}`}>
              {score.toFixed(1)}
              <span className="text-sm font-normal text-slate-400 ml-1">/100</span>
            </div>
          </div>

          <Badge variant="outline" className={`px-4 py-1 text-sm font-bold shadow-sm bg-white ${config.color} ${config.borderColor}`}>
            {status}
          </Badge>
          ...
          <p className="text-[10px] text-slate-500 leading-relaxed italic">
            *Status ini dihasilkan secara otomatis berdasarkan algoritma pembobotan kriteria survey Dompet Ummat.
          </p>
        </div>
      </CardContent>
    </Card>
