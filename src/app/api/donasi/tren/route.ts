...
export async function GET(req: Request) {
  ...
  // Validasi terhadap enum yang diketahui untuk mencegah SQL injection
  const VALID_PROGRAMS = [
    'Pendidikan', 'Kesehatan', 'Ekonomi',
    'Sosial Kemanusiaan', 'Dakwah & Advokasi', 'Operasional',
  ]
  const safeProgram = VALID_PROGRAMS.includes(program ?? '') ? program : null
  ...
  try {
    ...
    if (filterType === 'none' || filterType === 'year') {
      ...
      if (safeProgram) {
        rows = await prisma.$queryRaw<RawTrendRow[]>`
          SELECT
            SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 1, 4) AS tahun,
            SUM(fd.nominal_valid) AS total
          FROM fact_donasi fd
          JOIN dim_program_donasi dp ON dp.sk_program_donasi = fd.sk_program_donasi
          WHERE dp.program_induk = ${safeProgram}
            AND SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 1, 4) BETWEEN ${yearStart} AND ${yearEnd}
          GROUP BY SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 1, 4)
          ORDER BY tahun ASC
        `
      } else {
        rows = await prisma.$queryRaw<RawTrendRow[]>`
    ...
    if (filterType === 'month') {
      ...
      if (safeProgram) {
        rows = await prisma.$queryRaw<RawTrendRow[]>`
          SELECT
            SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 1, 4) AS tahun,
            SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 5, 2) AS bulan,
            SUM(fd.nominal_valid) AS total
          FROM fact_donasi fd
          JOIN dim_program_donasi dp ON dp.sk_program_donasi = fd.sk_program_donasi
          WHERE dp.program_induk = ${safeProgram}
            AND SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 1, 6) BETWEEN ${startMonthKey} AND ${endMonthKey}
          GROUP BY 1, 2
      ...
      if (safeProgram) {
        rows = await prisma.$queryRaw<RawTrendRow[]>`
          SELECT
            SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 1, 4) AS tahun,
            SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 5, 2) AS bulan,
            SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 7, 2) AS hari,
            SUM(fd.nominal_valid) AS total
          FROM fact_donasi fd
          JOIN dim_program_donasi dp ON dp.sk_program_donasi = fd.sk_program_donasi
          WHERE dp.program_induk = ${safeProgram}
            AND SUBSTRING(CAST(fd.sk_tgl_bersih AS CHAR), 1, 8) BETWEEN ${startDateKey} AND ${endDateKey}
          GROUP BY 1, 2, 3
    ...
    // ── Data mapping (tidak berubah dari semula) ──────────────────────────
    const data = rows.map((row) => {
      ...
      let label = tahun
      if (filterType === 'month' && bulan) label = `${MONTH_LABELS[bulan] ?? bulan} ${tahun}`
      if (filterType === 'day' && bulan && hari) label = `${hari} ${MONTH_LABELS[bulan] ?? bulan} ${tahun}`
    ...
    return NextResponse.json({ error: 'Gagal memuat tren donasi' }, { status: 500 })
  }
}