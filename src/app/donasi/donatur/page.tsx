...
export default function ManajemenDonaturPage() {
  ...
  const handleDelete = async (sk: number, nama: string) => {
    try {
      ...
      if (res.ok) {
        toast.success(`Data ${nama} dinonaktifkan`)
        fetchDonatur()
      }
  ...
  const currentDonatur = filteredDonatur.slice(indexOfFirstItem, indexOfLastItem)
        ...
        <Card className="border-slate-200 shadow-sm overflow-hidden bg-white">
          <CardHeader className="border-b py-4 bg-slate-50/50">
            <div className="relative max-w-sm">
              ...
              <Input placeholder="Cari nama, kontak, atau perusahaan..." className="pl-10 font-bold border-slate-200 focus:border-indigo-500 transition-all" value={search} 
                onChange={(e) => {
          ...
          <CardContent className="p-0">
                          ...
                          <AlertDialogContent className="rounded-2xl border-2 shadow-2xl">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription className="font-medium text-slate-500 text-sm leading-relaxed">
                                ...
                                Data ini tetap tersimpan di <strong>Data Warehouse</strong> untuk keperluan audit historis.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t bg-slate-50/30">
              <p className="text-xs font-bold text-slate-500">
                Menampilkan <span className="text-indigo-600">{indexOfFirstItem + 1}</span> - <span className="text-indigo-600">{Math.min(indexOfLastItem, filteredDonatur.length)}</span> dari <span className="text-indigo-600">{filteredDonatur.length}</span> Donatur
              </p>
                ...
                <div className="flex items-center gap-1">
                  {[...Array(totalPages)].map((_, i) => (
                    <Button
                      ...
                      className={`h-8 w-8 p-0 text-xs font-bold ${currentPage === i + 1 ? "bg-indigo-600" : ""}`}
                    >
                      {i + 1}
                    </Button>
